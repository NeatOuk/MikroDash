class VpnCollector {
  ...
  async safeGet(cmd) {
    try {
      const r = await this.ros.write(cmd);
  ...
  async tick() {
    ...
    if (!this._debuggedOnce && wgPeers.length > 0) {
      console.log(
        `[vpn] ${wgPeers.length} WireGuard peer(s) found on interfaces: ${[...new Set(wgPeers.map((p) => p.interface).filter(Boolean))].join(", ") || "?"}`,
      );
    ...
    const tunnels = wgPeers.map((p) => {
      ...
      const peerName =
        p.name && String(p.name).trim()
          ? String(p.name).trim()
          : p.comment && String(p.comment).trim()
            ? String(p.comment).trim()
            : p["allowed-address"] && String(p["allowed-address"]).trim()
              ? String(p["allowed-address"]).trim()
              : p["public-key"]
                ? p["public-key"].slice(0, 16) + "\u2026"
                : "?";
