  ...
  async tick() {
    ...
    if (detectedMode === "wifi" || detectedMode === null) {
      try {
        ...
        // L-3: Log at debug level so silent failures are visible when debugging.
        if (process.env.ROS_DEBUG === "true")
          console.debug(
            "[wireless] wifi probe failed:",
    ...
    if (
      !clients.length &&
      (detectedMode === "wireless" || detectedMode === null)
    ) {
      try {
        const res = await this.ros.write(
        ...
        if (process.env.ROS_DEBUG === "true")
          console.debug(
            "[wireless] wireless probe failed:",
            e && e.message ? e.message : e,
          );
      }
    }
    ...
    const parsed = clients
      .map((c) => {
        ...
        const signal = parseInt(
          c.signal || c["signal-strength"] || c["rx-signal"] || "0",
          10,
        );
        ...
        let band = "";
        const ifLow = iface.toLowerCase(),
          txLow = txRate.toLowerCase();
        if (
          /wifi[12]|wlan[12]|5g|5ghz|ax5/.test(ifLow) ||
          /[MW]HT-[4-9]\d{2}|[MW]HT-[1-9]\d{3}|HE-MCS/.test(txRate)
        )
          band = "5GHz";
