...
class DhcpNetworksCollector {
  ...
  async tick() {
    ...
    for (const a of addrRows) {
      if (a.interface === wanIface && a.address) {
        wanIp = a.address;
        break;
      }
    }
    ...
    for (const n of netRows) {
      ...
      const leaseCount = leaseIps.reduce(
        (acc, ip) => acc + (ipInCidr(ip, n.address) ? 1 : 0),
        0,
      );
      networks.push({
        cidr: n.address,
    ...
    // L-4: Apply null-guard consistently for all this.state accesses.
    if (this.state) this.state.lastWanIp = wanIp;
    this.io.emit("lan:overview", {
      ts: Date.now(),
      lanCidrs: this.lanCidrs,
      networks: this.networks,
      wanIp,
    });
    if (this.state) this.state.lastNetworksTs = Date.now();
  }
  ...
  start() {
    const run = async () => {
      try {
        ...
        console.error("[dhcp-networks]", e && e.message ? e.message : e);
      }
    };
    run();
