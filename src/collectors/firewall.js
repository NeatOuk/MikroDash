class FirewallCollector {
  ...
  processChain(rules) {
    return rules
      .filter((r) => r.disabled !== "true" && r.disabled !== true)
      .map((r) => {
        const id = r[".id"] || "";
  ...
  async tick() {
    ...
    // H-5: Prune stale prevCounts BEFORE emitting so the emitted data is consistent.
    const seenIds = new Set(
      [...filterRules, ...natRules, ...mangleRules]
        .map((r) => r.id)
        .filter(Boolean),
    );
    ...
    const topByHits = [...filterRules, ...natRules, ...mangleRules]
