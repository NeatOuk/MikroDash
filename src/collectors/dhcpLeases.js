  ...
  _applyLease(l) {
    ...
    const name =
      l.comment && l.comment.trim()
        ? l.comment.trim()
        : l["host-name"] && l["host-name"].trim()
          ? l["host-name"].trim()
          : "";
    const status = l.status || "";

    // H-1: Prune expired/removed leases from maps to prevent unbounded growth.
    const stale = status === "expired" || status === "removed";
    if (stale) {
