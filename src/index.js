...
function _freshState() {
  return {
    ...
    lastSystemErr: null,
    lastWirelessTs: 0,
    lastWirelessErr: null,
    lastVpnTs: 0,
    lastVpnErr: null,
    lastFirewallTs: 0,
    lastFirewallErr: null,
    lastIfStatusTs: 0,
    lastPingTs: 0,
    lastRoutingTs: 0,
    lastRoutingErr: null,
    lastBandwidthTs: 0,
    lastBandwidthErr: null,
  };
}
...
function buildSession(routerCfg) {
  ...
  const connTableCache = {
    ...
    async get(rosInst) {
      ...
      this.rows =
        (await rosInst.write("/ip/firewall/connection/print", [
  ...
  const dhcpNetworks = new DhcpNetworksCollector({
    ros,
    io,
    pollMs: _cfg.pollDhcp,
    dhcpLeases,
    state,
    wanIface: DEFAULT_IF,
  });
  ...
  const conns = new ConnectionsCollector({
    ros,
    io,
    pollMs: _cfg.pollConns,
    topN: _cfg.topN,
    maxConns: _cfg.maxConns,
    dhcpNetworks,
    dhcpLeases,
    arp,
    state,
  ...
  const talkers = new TopTalkersCollector({
    ros,
    io,
    pollMs: _cfg.pollTalkers,
    state,
    topN: _cfg.topTalkersN,
  });
  const logs = new LogsCollector({ ros, io, state });
  const system = new SystemCollector({
    ros,
    io,
    pollMs: _cfg.pollSystem,
    state,
  });
  const wireless = new WirelessCollector({
    ros,
    io,
    pollMs: _cfg.pollWireless,
    state,
    dhcpLeases,
    arp,
  });
  const vpn = new VpnCollector({ ros, io, pollMs: _cfg.pollVpn, state });
  const firewall = new FirewallCollector({
    ros,
    io,
    pollMs: _cfg.pollFirewall,
    state,
    topN: _cfg.firewallTopN,
  });
  const ifStatus = new InterfaceStatusCollector({
    ros,
    io,
    pollMs: _cfg.pollIfstatus,
    state,
  });
  const ping = new PingCollector({
    ros,
    io,
    pollMs: _cfg.pollPing,
    state,
    target: PING_TARGET,
  });
  ...
  const routing = new RoutingCollector({
    ros,
    io,
    pollMs: _cfg.pollRouting,
    state,
  });
...
async function teardownSession(session) {
  ...
  session.ros.removeAllListeners();
  session.ros.stop();
...
function wireRosEvents(session) {
  ...
  ros.on("connected", () => {
    console.log(
      `[ROS] ✓ connected to ${host}:${port} as "${user}" (${tls ? "TLS" : "plain"})`,
    );
...
async function startCollectors(session) {
  ...
  try {
    console.log(
      `[MikroDash] v${APP_VERSION} — RouterOS connected, starting collectors`,
    );
    ...
    session.system._onFirstBoardName = (boardName) => {
      ...
      if (
        router &&
        (router.label === "My Router" || router.label === router.host)
      ) {
    ...
    for (const [, socket] of io.sockets.sockets) {
      ...
      sendInitialState(socket).catch((e) => {
        console.error(
          "[MikroDash] sendInitialState failed for socket",
          socket.id,
          ":",
    ...
    _collectorsStarted = false;
    console.error(
      "[MikroDash] Collector startup error:",
      e && e.message ? e.message : e,
    );
  }
}
...
async function switchRouter(newRouterId) {
  ...
  try {
    console.log(
      `[MikroDash] Switching to router: ${router.label} (${router.host})`,
    );
...
(function bootstrap() {
  ...
  if (!activeId) {
    console.error(
      "[MikroDash] No routers configured. Add a router in Settings.",
    );
  ...
  try {
    ...
    if (body._reset) {
      ...
      io.emit("settings:pages", {
        pageWireless: DEFAULTS.pageWireless,
        pageInterfaces: DEFAULTS.pageInterfaces,
        pageDhcp: DEFAULTS.pageDhcp,
        pageVpn: DEFAULTS.pageVpn,
        pageConnections: DEFAULTS.pageConnections,
        pageFirewall: DEFAULTS.pageFirewall,
        pageLogs: DEFAULTS.pageLogs,
        pageBandwidth: DEFAULTS.pageBandwidth,
        pageRouting: DEFAULTS.pageRouting,
        alertCpuThreshold: DEFAULTS.alertCpuThreshold,
        alertPingLoss: DEFAULTS.alertPingLoss,
      });
    ...
    const intFields = {
      routerPort: [1, 65535],
      pollConns: [500, 60000],
      pollTalkers: [500, 60000],
      pollSystem: [500, 60000],
      pollWireless: [500, 60000],
      pollVpn: [500, 30000],
      pollFirewall: [500, 30000],
      pollIfstatus: [500, 60000],
      pollPing: [500, 30000],
      pollArp: [5000, 300000],
      pollBandwidth: [500, 60000],
    ...
    for (const [f, range] of Object.entries(intFields)) {
      if (f in body) {
        ...
        if (!isNaN(v) && v >= range[0] && v <= range[1]) updates[f] = v;
      }
    }
    for (const f of strFields) {
      if (f in body) updates[f] = String(body[f]).trim().slice(0, 256);
    }
    for (const f of boolFields) {
      if (f in body) updates[f] = body[f] === true || body[f] === "true";
    }
    for (const f of credFields) {
      if (f in body && !Settings.isMasked(body[f]))
        updates[f] = String(body[f]).slice(0, 512);
    }
    ...
    const pollMap = {
      ...
      pollTalkers: "talkers",
      pollSystem: "system",
      pollWireless: "wireless",
      pollVpn: "vpn",
      pollFirewall: "firewall",
      pollIfstatus: "ifStatus",
      pollBandwidth: "bandwidth",
      pollPing: "ping",
      pollArp: "arp",
      pollDhcp: "dhcpNetworks",
      pollRouting: "routing",
    };
    ...
    const pageSettings = {
      pageWireless: saved.pageWireless,
      pageInterfaces: saved.pageInterfaces,
      pageDhcp: saved.pageDhcp,
      pageVpn: saved.pageVpn,
      pageConnections: saved.pageConnections,
      pageFirewall: saved.pageFirewall,
      pageLogs: saved.pageLogs,
      pageBandwidth: saved.pageBandwidth,
      pageRouting: saved.pageRouting,
      alertCpuThreshold: saved.alertCpuThreshold,
      alertPingLoss: saved.alertPingLoss,
    };
...
app.post("/api/routers", (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      router: { ...router, password: router.password ? "••••••••" : "" },
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: String(e.message || e) });
  }
  ...
  try {
    ...
    if (req.params.id === _cfg.activeRouterId) {
      return res.status(409).json({
        ok: false,
        error:
          "Cannot delete the active router. Switch to a different router first.",
      });
    }
...
app.post("/api/routers/test", async (req, res) => {
  ...
  const timeout = setTimeout(
    () => done(false, "Connection timed out after 8 seconds"),
    9000,
  );
  ...
  testRos.on("connectionError", (e) => {
    ...
    let reason = msg;
    if (/ECONNREFUSED/.test(msg))
      reason = "Connection refused — check host and port";
    else if (/ETIMEDOUT/.test(msg) || /timed out/i.test(msg))
      reason = "Connection timed out — check host and firewall rules";
    else if (/ENOTFOUND/.test(msg) || /ENOENT/.test(msg))
      reason = "Host not found — check router host/IP";
    else if (/ECONNRESET/.test(msg)) reason = "Connection reset by router";
    else if (/certificate/i.test(msg))
      reason = 'TLS certificate error — try enabling "Allow self-signed cert"';
    else if (/authentication/i.test(msg) || /login/i.test(msg))
      reason = "Authentication failed — check username and password";
    else if (body.tls && /RosException/.test(msg))
      reason =
        "TLS handshake failed — check that RouterOS api-ssl is enabled and the certificate is valid.";
    done(false, reason);
  });
...
async function sendInitialState(socket) {
  ...
  socket.emit("traffic:history", {
    ...
    points: s.traffic.hist.get(s.DEFAULT_IF)
      ? s.traffic.hist.get(s.DEFAULT_IF).toArray()
      : [],
  });
  ...
  if (!s.ros.connected) {
    ...
    try {
      await s.ros.waitUntilConnected(10000);
    } catch (_) {}
  }
  ...
  try {
    ...
    console.error(
      "[MikroDash] fetchInterfaces failed for socket",
      socket.id,
      ":",
      reason,
    );
  ...
  if (!_wanIp && s.ifStatus.lastPayload) {
    ...
    const _match = (s.ifStatus.lastPayload.interfaces || []).find(
      (i) =>
        i.name && i.name.toLowerCase() === _wanIface && i.ips && i.ips.length,
    );
  ...
  const allLeases = [];
  for (const [ip, v] of s.dhcpLeases.byIP.entries())
    allLeases.push({ ip, ...v });
  socket.emit("leases:list", { ts: Date.now(), leases: allLeases });

  if (s.wireless.lastPayload)
    socket.emit("wireless:update", s.wireless.lastPayload);
  ...
  if (s.ifStatus.lastPayload)
    socket.emit("ifstatus:update", s.ifStatus.lastPayload);
  if (s.firewall.lastPayload)
    socket.emit("firewall:update", s.firewall.lastPayload);
  if (s.conns.lastPayload) socket.emit("conn:update", s.conns.lastPayload);
  if (s.talkers.lastPayload)
    socket.emit("talkers:update", s.talkers.lastPayload);
  ...
  if (s.routing.lastPayload)
    socket.emit("routing:update", s.routing.lastPayload);

  const _ps = Settings.load();
  socket.emit("settings:pages", {
    pageWireless: _ps.pageWireless,
    pageInterfaces: _ps.pageInterfaces,
    pageDhcp: _ps.pageDhcp,
    pageVpn: _ps.pageVpn,
    pageConnections: _ps.pageConnections,
    pageFirewall: _ps.pageFirewall,
    pageLogs: _ps.pageLogs,
    pageBandwidth: _ps.pageBandwidth,
    pageRouting: _ps.pageRouting,
    alertCpuThreshold: _ps.alertCpuThreshold,
    alertPingLoss: _ps.alertPingLoss,
  });
...
io.on("connection", (socket) => {
  if (io.engine.clientsCount > MAX_SOCKETS) {
    console.warn(
      "[MikroDash] connection rejected — max sockets reached:",
      MAX_SOCKETS,
    );
...
server.listen(PORT, () =>
  console.log(
    `[MikroDash] v${APP_VERSION} listening on http://0.0.0.0:${PORT}`,
  ),
);
