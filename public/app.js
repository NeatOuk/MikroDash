...
function actionBadge(a) {
  var c = "secondary";
  if (a === "accept" || a === "passthrough") c = "success";
  else if (a === "drop" || a === "reject" || a === "tarpit") c = "danger";
  else if (a === "log" || a === "add-src-to-address-list") c = "warning";
  else if (a === "masquerade" || a === "dst-nat" || a === "src-nat") c = "info";
  return (
    '<span class="badge bg-' +
    c +
    '" style="font-family:var(--font-mono);font-size:.63rem;color:#000">' +
    esc(a) +
    "</span>"
  );
}
...
function gauge(label, pct, cls) {
  ...
  return (
    '<div class="gauge-item"><div class="gauge-label">' +
    esc(label) +
    "</div>" +
...
socket.on("system:update", function (d) {
  ...
  if (d.cpuFreq)
    meta +=
      '<div class="sys-meta-item"><strong>' + d.cpuFreq + "</strong> MHz</div>";
  if (d.tempC != null)
    meta +=
      '<div class="sys-meta-item"><strong>' +
      d.tempC +
  ...
  if (rosUpdateRow) {
    ...
    if (d.updateAvailable && d.latestVersion) {
      ...
      ur =
        '<div class="ros-update-row pending"><span class="ros-update-dot"></span>' +
        esc(d.updateStatus) +
        "</div>";
    } else {
      ur =
  ...
  if (data.topDestinations && data.topDestinations.length) {
    topDests.innerHTML = data.topDestinations
      .map(function (d) {
        ...
        return (
          '<div class="top-row">' +
          '<div class="top-name text-truncate" style="flex:1;min-width:0">' +
          esc(d.key) +
          "</div>" +
...
socket.on("talkers:update", function (data) {
  ...
  if (ndWired) ndWired.textContent = devices.length;
  if (!devices.length) {
...
(function () {
  ...
  socket.on("wireless:update", function (data) {
    ...
    var badgeCls =
      "badge " + (_wlClients.length > 0 ? "bg-blue" : "bg-secondary");
    wirelessTabBadge.textContent = _wlClients.length;
    wirelessTabBadge.className = badgeCls;
...
socket.on("vpn:update", function (data) {
  var peers = (data.tunnels || []).filter(function (t) {
    return t.type === "WireGuard" && t.state === "connected";
  });
  vpnCount.textContent = peers.length;
  vpnCount.className =
    "badge " + (peers.length > 0 ? "bg-green" : "bg-secondary");
  var nb = $("vpnNavBadge");
  if (nb) nb.textContent = peers.length;
  // Dashboard mini card
  if (!peers.length) {
    vpnTable.innerHTML =
      '<tr><td colspan="3" class="empty-state">No active peers</td></tr>';
  } else {
    vpnTable.innerHTML = peers
      .map(function (t) {
  ...
  var allPeers = (data.tunnels || []).filter(function (t) {
      ...
      grid.innerHTML = allPeers
        .map(function (t) {
          ...
          return (
            ...
            (connected && t.uptime && t.uptime !== "never"
              ? "HS: " + esc(t.uptime)
              : "Never connected") +
            "</div>" +
            "</div>"
...
function renderDhcp(leases) {
  ...
  if (dhcpTotalBadge) {
    ...
    dhcpTotalBadge.className =
      "badge " + (count > 0 ? "bg-blue" : "bg-secondary");
...
function renderFirewallTab() {
  ...
  if (!rules.length) {
    firewallTable.innerHTML =
      '<tr><td colspan="5" class="empty-state">No rules with hits</td></tr>';
    return;
  }
...
function checkPingNotif(loss, target) {
  var now = Date.now();
  if (loss === 100 && now - _pingAlertedAt > NOTIF_COOLDOWN) {
    // L-5: Use the actual target from the event payload when available.
    var host =
      target && String(target).trim() ? String(target).trim() : "1.1.1.1";
    sendNotif(
      "Ping Loss",
      "No response from " + host + " — possible WAN outage",
      "ping-loss",
    );
    _pingAlertedAt = now;
  }
...
(function () {
  ...
  socket.on("ping:update", function (data) {
    checkPingNotif(data.loss, data.target);
  });
})();
...
(function () {
  ...
  function bindTooltip() {
    mapEl.addEventListener("mousemove", function (e) {
      var tgt = e.target;
      if (!tgt.dataset || !tgt.dataset.cc) return;
      ...
      tooltipEl.innerHTML =
        ...
        (city ? " · " + esc(city) : "") +
        (n
          ? ' &nbsp;<span style="color:var(--accent-rx)">' + n + " conns</span>"
          : "") +
        (proto.tcp || proto.udp
      ...
      var tx = e.clientX - wrap.left + 10,
        ty = e.clientY - wrap.top - 30;
    ...
    .then(function (world) {
      ...
      s.onload = function () {
        ...
        (function () {
          ...
          function setFsBtnVisible() {
            if (fsBtn) fsBtn.style.display = isMobile() ? "flex" : "none";
          }
          setFsBtnVisible();
